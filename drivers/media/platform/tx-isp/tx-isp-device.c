...
#define JOINT_GROUP_MAX 4
int joint_sizeimage[JOINT_GROUP_MAX] = {0};
int joint_vb_chn[JOINT_GROUP_MAX] = {0};
int joint_group_num = 0;
...
enum{
	...
	PRO_ZRT_IOCTL_GET_JOINTSIZE_BY_IDX,
	PRO_ZRT_IOCTL_GET_JOINT_VB_CHN_BY_IDX,
	PRO_ZRT_IOCTL_GET_I2D_POOL_SIZE,
	PRO_ZRT_IOCTL_GET_JOINT_GROUP_NUM,
	PRO_ZRT_IOCTL_INVALUE,
};
#define ZRT_IOCTL_TYPE 'Z'
#define ZRT_IOCTL_READ				_IOR(ZRT_IOCTL_TYPE, PRO_ZRT_IOCTL_TYPE, int)
#define ZRT_IOCTL_GET_JOINTSIZE_BY_IDX	_IOR(ZRT_IOCTL_TYPE, PRO_ZRT_IOCTL_GET_JOINTSIZE_BY_IDX, int)
#define ZRT_IOCTL_GET_JOINT_VB_CHN_BY_IDX	_IOR(ZRT_IOCTL_TYPE, PRO_ZRT_IOCTL_GET_JOINT_VB_CHN_BY_IDX, int)
#define ZRT_IOCTL_GET_I2D_POOL_SIZE _IOR(ZRT_IOCTL_TYPE, PRO_ZRT_IOCTL_GET_I2D_POOL_SIZE, int)
#define ZRT_IOCTL_GET_JOINT_GROUP_NUM _IOR(ZRT_IOCTL_TYPE, PRO_ZRT_IOCTL_GET_JOINT_GROUP_NUM, int)
...
static long zeratul_variable_proc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ZRT_IOCTL_GET_I2D_POOL_SIZE:
		...
		break;
	case ZRT_IOCTL_GET_JOINT_GROUP_NUM:
		{
			if (copy_to_user((int __user *)arg, &joint_group_num, sizeof(joint_group_num)))
				return -EFAULT;
		}
		break;
	case ZRT_IOCTL_GET_JOINTSIZE_BY_IDX:
		{
			int idx = 0;
			int value = 0;
			if (copy_from_user(&idx, (int __user *)arg, sizeof(idx)))
				return -EFAULT;
			if(idx >= 0 && idx < JOINT_GROUP_MAX)
				value = joint_sizeimage[idx];
			if (copy_to_user((int __user *)arg, &value, sizeof(value)))
				return -EFAULT;
		}
		break;
	case ZRT_IOCTL_GET_JOINT_VB_CHN_BY_IDX:
		{
			int idx = 0;
			int value = -1;
			if (copy_from_user(&idx, (int __user *)arg, sizeof(idx)))
