...
static void jz_mac_invalidate_link_state(struct jz_mac_local *lp)
{
	lp->old_link = 0;
	lp->old_speed = 0;
	lp->old_duplex = -1;
...
static int mii_probe(struct net_device *dev)
{
	...
	jz_mac_invalidate_link_state(lp);
	lp->phydev = phydev;
...
static int jz_mac_open(struct net_device *dev)
{
	...
	/*
	 * The GMAC registers are reset on every reopen. Drop the cached PHY
	 * state so adjust_link() always re-programs speed/duplex afterwards.
	 */
#ifdef CONFIG_PM
	jz_mac_invalidate_link_state(lp);
#endif/*CONFIG_PM*/
...
static int jz_mac_probe(struct platform_device *pdev)
{
	...
	pr_debug("=======>gmacdev = 0x%08x<================\n", (u32)gmacdev);
	synopGMAC_multicast_enable(gmacdev);
	if (gmacdev) {
		pr_debug("=========>gmacdev->MacBase = 0x%08x DmaBase = 0x%08x\n",
...
static int jz_mac_resume(struct platform_device *pdev)
{
	...
	if (ret < 0)
		return ret;
	/*
	 * The board-specific helper only keeps reset asserted; wait once the
	 * line is released so the PHY is ready before MDIO access resumes.
	 */
	msleep(100);
#endif
	if (netif_running(net_dev)) {
		ret = jz_mac_open(net_dev);
		if (ret)
			return ret;
...
static ssize_t mdio_cmd_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(buf, "r reg", sizeof("r reg")-1)) {
		...
		if (ret)
			printk("##### err %s.%d\n", __func__, __LINE__);
		pr_debug("mdio: reg read 0x%x-0x%x:0x%x\n", phybase, regoffset, data);
		...
		if (3!=ret)
			return EFAULT;
		pr_debug("mdio: reg write 0x%x-0x%x-0x%x\n", phybase, regoffset, data);
		{
