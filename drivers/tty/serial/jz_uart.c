...
static void dma_receive_chars(struct uart_jz47xx_port *up, int lsr, int residue, int count)
{
    ...
    /* Sanity check */
    if (!up || !up->chan_rx || !up->port.state || !up->port.state->port.tty)
        return;
...
static bool filter_tx(struct dma_chan *chan, void *data)
{
    struct uart_jz47xx_port *port = data;
    /* TX uses dma_type */
    return (void*)port->dma_type == chan->private;
...
static bool filter_rx(struct dma_chan *chan, void *data)
{
    struct uart_jz47xx_port *port = data;
    /* RX uses dma_type + 1 */
    return (void*)(port->dma_type + 1) == chan->private;
}
...
static void serial_jz47xx_dma_init(struct uart_jz47xx_port *up)
{
    ...
    /* Request RX channel first (dma_type + 1) */
    up->chan_rx = dma_request_channel(mask, filter_rx, up);
    if (!up->chan_rx) {
        dev_err(up->port.dev, "%s: dma_request_channel FAILS(rx)\n", __func__);
        return;
    }

    /* Request TX channel (dma_type) */
    up->chan_tx = dma_request_channel(mask, filter_tx, up);
    if (!up->chan_tx) {
...
static inline irqreturn_t serial_jz47xx_irq(int irq, void *dev_id)
{
    ...
    if (iir & UART_IIR_NO_INT)
        return IRQ_NONE;
    if(up->use_dma) {
        /* In DMA mode, RX is handled by DMA.
         * Only handle PIO fallback for overflow cases.
         */
        if(lsr & UART_LSR_DR)
...
static void serial_jz47xx_shutdown(struct uart_port *port)
{
    ...
    if(up->use_dma) {
        ...
        if (up->chan_rx)
            dmaengine_terminate_all(up->chan_rx);
        if (up->chan_tx)
            dmaengine_terminate_all(up->chan_tx);
...
static int serial_jz47xx_probe(struct platform_device *dev)
{
    ...
    if (dma_filter) {
        ...
        if (up->chan_rx && up->chan_tx) {
            /* DMA channels acquired successfully */
            up->use_dma = 1;
            up->buf_id = 0;
            ...
            dev_info(&dev->dev, "UART%d DMA mode enabled\n", dev->id);
        } else {
            /* DMA init failed, fall back to PIO mode */
            dev_warn(&dev->dev, "UART%d DMA init failed, using PIO mode\n", dev->id);
            up->use_dma = 0;
            ...
            if (up->rx_buf_virt) {
                dma_free_coherent(&dev->dev, COUNT_DMA_BUFFER, up->rx_buf_virt, up->rx_buf_dma);
                up->rx_buf_virt = NULL;
            }
            kfree(up->desc_tx);
            kfree(up->desc_rx);
