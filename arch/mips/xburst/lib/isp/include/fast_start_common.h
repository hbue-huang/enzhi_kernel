...
#define RMEM_ADDR_OVERFLOW_CHECK(addr, max_addr) \
	do { \
		if ((addr) > (max_addr)) { \
			printk(KERN_ERR "%s:%d rmem Address overflow: addr=0x%x, max_addr=0x%x\n", \
			       __func__, __LINE__, (unsigned int)(addr), (unsigned int)(max_addr)); \
		} \
	} while (0)
...
extern int joint_sizeimage[JOINT_GROUP_MAX];
extern int joint_vb_chn[JOINT_GROUP_MAX];
extern int joint_group_num;
