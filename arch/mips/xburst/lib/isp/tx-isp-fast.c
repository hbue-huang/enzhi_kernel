...
int enc_dqbuf(int chnNum,FrameInfo *frame)
{
	...
	if((g_chn[chnNum].joint_handler) != NULL){
		...
		if (g_chn[chnNum].ldc_module_attr.i2d_enable == 1){
#if FAST_LDC_OFFLINE
			fast_set_ldc(chnNum,buf->index);
			frame->phyAddr = g_chn[chnNum].vb_buf[buf->index].paddr;
#endif
		}else{
...
static int enc_video(unsigned int addr)
{
	...
	int enc_size = 0;
	int joint_group0_chn[4] = {0,3};
	int joint_group1_chn[4] = {1,4};

	VPU_Kern_Init();
#if JOINT_MODE
	ret = VPU_Encoder_SetJointAttr(joint_group0_chn, 2);
	ret = VPU_Encoder_SetJointAttr(joint_group1_chn, 2);
	if(ret < 0){
	...
	RMEM_ADDR_OVERFLOW_CHECK(bs_addr+vpuBs_size, rmem_base + rmem_size);
		...
		if( ! IS_ERR_OR_NULL(chan)){
			...
			if(chan->state == TX_ISP_MODULE_RUNNING){
				...
				if((g_chn[chnNum].joint_handler) != NULL){
					...
					if (g_chn[chnNum].ldc_module_attr.i2d_enable == 1){
						if ((g_chn[chnNum].i2dattr.rotate_angle == 90) || (g_chn[chnNum].i2dattr.rotate_angle == 270)) {
							chnAttr.picWidth = frame_channel_height[chnNum];
							chnAttr.picHeight = frame_channel_width[chnNum];
						}else{
				...
				RMEM_ADDR_OVERFLOW_CHECK(chnAttr.encPhyaddr, rmem_base + rmem_size);
...
{
	RMEM_ADDR_OVERFLOW_CHECK(addr, rmem_base + rmem_size);
#if FAST_LDC_OFFLINE
...
//	if(chn_num == 2  || chn_num == 5){
	...
	/* Find or assign joint group index for multi-joint support */
	if((g_chn[chn_num].joint_handler) != NULL){
		if(g_chn[chn_num].joint_handler->buf[0].paddr == 0){
			/* New joint group, assign a new index */
			if(joint_group_num < JOINT_GROUP_MAX){
				joint_idx = joint_group_num;
				joint_group_num++;
				...
				joint_sizeimage[joint_idx] = g_chn[chn_num].joint_handler->sizeimage;
				joint_vb_chn[joint_idx] = chn_num;
				printk("joint_idx:%d joint_vb_chn:%d width_max:%d height_max%d joint_sizeimage:%d\n", joint_idx, joint_vb_chn[joint_idx], g_chn[chn_num].joint_handler->width_max, (g_chn[chn_num].joint_handler->height_max+15)&~15, joint_sizeimage[joint_idx]);
			}else{
				printk(KERN_ERR "Exceeded maximum joint group number:%d\n", JOINT_GROUP_MAX);
			}
		}else{
			/* Find existing joint group index */
			for(i = 0; i < joint_group_num; i++){
				if(joint_vb_chn[i] == chn_num || g_chn[joint_vb_chn[i]].joint_handler == g_chn[chn_num].joint_handler){
					joint_idx = i;
					break;
				}
			}
	...
	for(buf_i = 0; buf_i < count; buf_i++) {
		...
		switch(direct_mode){
			...
			case 0:
				if((g_chn[chn_num].joint_handler) != NULL){
					if(g_chn[chn_num].joint_handler->buf[buf_i].paddr == 0){
						/* Allocate buffer for current buf_i */
						g_chn[chn_num].joint_handler->buf[buf_i].paddr = addr + buf_i * joint_sizeimage[joint_idx];
						g_chn[chn_num].joint_handler->buf[buf_i].vaddr = ioremap(g_chn[chn_num].joint_handler->buf[buf_i].paddr, joint_sizeimage[joint_idx]);
						memset(g_chn[chn_num].joint_handler->buf[buf_i].vaddr, 0, joint_sizeimage[joint_idx]*2/3);
						memset(g_chn[chn_num].joint_handler->buf[buf_i].vaddr + joint_sizeimage[joint_idx]*2/3, 128, joint_sizeimage[joint_idx]/3);
	    ...
	    (direct_mode == 4 && (chn_num == 0 || chn_num == 3 || chn_num == 6 || chn_num == 9)))
	{
		...
		//undirect
		if((g_chn[chn_num].joint_handler) != NULL){
			/* Update address based on joint group allocation */
			if(joint_idx >= 0 && joint_idx < JOINT_GROUP_MAX){
				addr = addr + count * joint_sizeimage[joint_idx];
			}
		}else{
...
{
	int ret = 0;
	ret = fast_sensors[use_num_sensor - 1].init_sensor();
	if(ret < 0) {
		ISP_WARNING("init_sensor %s failed\n", fast_sensors[use_num_sensor - 1].name);
		return ret;
	}
	return 0;
}
static int all_sensor_info_init(int sensor_number)
{
	...
	case 1:
		if(sensor_number == 1 && use_num_sensor > 0){
			strcpy(g_sensor_info[0].name, fast_sensors[use_num_sensor - 1].name);
			strcpy(g_sensor_info[0].i2c.type, fast_sensors[use_num_sensor - 1].name);
			g_sensor_info[0].i2c.addr = fast_sensors[use_num_sensor - 1].i2c_addr;
		}else{
...
{
	int ret = 0;
	int timeout = 200, init_sensor_num = 0;
	memset(&fast_sensors,0,sizeof(fast_sensors));

	if(sensor_number == 1 && use_num_sensor > 0)
		init_sensor_num = use_num_sensor;
	else
		init_sensor_num = sensor_number;
	switch(init_sensor_num){
	case 4:
...
int set_ipc_buf(unsigned int rmem_addr)
{
	...
	RMEM_ADDR_OVERFLOW_CHECK(rmem_addr, rmem_base + rmem_size);
...
{	//TO DO
	if(plcd_attr){
		memset(plcd_attr, 0 ,sizeof(struct t32_ldc_online_init));
	}
	...
	if (sensor_wdr_mode) {
		if(user_wdr_mode == 1) {
			...
			if(g_is_night == IR_STATUS_NIGHT){
				...
				get_sensor_setting_md5(st_bin_attr.vinum,iq_index,TISP_WDR_NIGHT);
			}
			else{
				...
				get_sensor_setting_md5(st_bin_attr.vinum,iq_index,TISP_WDR_DAY);
			}
...
//          tx_isp_unlocked_ioctl_first(TISP_VIDIOC_SET_ISP_WDR_OPEN,.....);
			...
			if(g_is_night == IR_STATUS_NIGHT){
				...
				get_sensor_setting_md5(st_bin_attr.vinum,iq_index,TISP_LINEAR_NIGHT);
			}
			else{
				st_bin_attr.dn_mode = TISP_DN_DAY;
				...
				get_sensor_setting_md5(st_bin_attr.vinum,iq_index,TISP_LINEAR_DAY);
			}
            tx_isp_unlocked_ioctl_first(TISP_VIDIOC_SET_TIZIANO_PAR,(unsigned long)&st_bin_attr);
		}
	}else{
			...
			for ( i=0; i < sensor_number; i++) {
				...
				if(g_is_night == IR_STATUS_NIGHT){
					st_bin_attr.dn_mode = TISP_DN_NIGHT;
					if(st_bin_attr.vinum == 0 && use_num_sensor > 0){
						st_bin_attr.paddr = virt_to_phys((void *)get_sensor_setting(use_num_sensor - 1,iq_index,TISP_LINEAR_NIGHT));
						st_bin_attr.size = get_sensor_setting_len(use_num_sensor - 1,iq_index,TISP_LINEAR_NIGHT);
						get_sensor_setting_md5(use_num_sensor - 1,iq_index,TISP_LINEAR_NIGHT);
					}else{
						st_bin_attr.paddr = virt_to_phys((void *)get_sensor_setting(i,iq_index,TISP_LINEAR_NIGHT));
						st_bin_attr.size = get_sensor_setting_len(i,iq_index,TISP_LINEAR_NIGHT);
						get_sensor_setting_md5(i,iq_index,TISP_LINEAR_NIGHT);
					}
				}
				else{
					st_bin_attr.dn_mode = TISP_DN_DAY;
					if(st_bin_attr.vinum == 0 && use_num_sensor > 0){
						st_bin_attr.paddr = virt_to_phys((void *)get_sensor_setting(use_num_sensor - 1,iq_index,TISP_LINEAR_DAY));
						st_bin_attr.size = get_sensor_setting_len(use_num_sensor - 1,iq_index,TISP_LINEAR_DAY);
						get_sensor_setting_md5(use_num_sensor - 1,iq_index,TISP_LINEAR_DAY);
					}else{
						st_bin_attr.paddr = virt_to_phys((void *)get_sensor_setting(i,iq_index,TISP_LINEAR_DAY));
						st_bin_attr.size = get_sensor_setting_len(i,iq_index,TISP_LINEAR_DAY);
						get_sensor_setting_md5(i,iq_index,TISP_LINEAR_DAY);
					}
				}
				tx_isp_unlocked_ioctl_first(TISP_VIDIOC_SET_TIZIANO_PAR,(unsigned long)&st_bin_attr);
			}
...
#if JOINT_MODE
	IMPFSJointAttr user_joint[2];
	memset(user_joint, 0xff, sizeof(user_joint));
	user_joint[0].position[0][0] = 0;
	user_joint[0].position[1][0] = 3;
	user_joint[1].position[0][0] = 1;
	user_joint[1].position[1][0] = 4;
	ret = fast_framesource_createjoint(&user_joint[0]);
	if(ret < 0){
		pr_err("create joint group:0 failed\n");
		return -1;
	}
	ret = fast_framesource_createjoint(&user_joint[1]);
	if(ret < 0){
		pr_err("create joint group:1 failed\n");
		return -1;
	}
#endif
	//sensor Stream on
